...
function activateCloudAccount(user){ if(!user?.id) return; const userId=user.id; const linkKey=accountStoreKey(LS_LINKS,userId); const saved=STORE.get(linkKey,null); if(saved===null){ const guestLinks=normalizeLinks(STORE.get(LS_LINKS,[])); links=guestLinks.map(link=>({ ...link, id:uid() })); cats=normalizeCategories(STORE.get(LS_CATS,DEFAULT_CATS)); protectedCats=new Set(STORE.get(LS_PROTECTED_CATS,DEFAULT_CATS.filter(c=>c!==OTHER_CAT))); persistAccountData(userId); } else { links=normalizeLinks(saved); cats=normalizeCategories(STORE.get(accountStoreKey(LS_CATS,userId),DEFAULT_CATS)); protectedCats=new Set(STORE.get(accountStoreKey(LS_PROTECTED_CATS,userId),DEFAULT_CATS.filter(c=>c!==OTHER_CAT))); } STORE.set(LS_ACTIVE_CLOUD_USER,userId); currentCat='all'; selectedIds.clear(); saveLocal(); renderTabs(); renderList(); }
